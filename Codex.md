...
- Chromium tab navigation runs through the injected WebDriver BiDi mapper using `browsingContext.navigate` in both the normal plugin path and the standalone web-surface runtime. CDP may identify the target's frame/context during lifecycle setup, but must not perform navigation.
- Chromium injects the pinned `chromium-bidi` mapper artifact into a hidden mapper target and uses it for navigation and subsequent page behavior.
...
- Web hook types include new-page, file-chooser, and download. New-page snapshots top-level page ids and records the source page. File-chooser subscribes to the WebDriver BiDi `input.fileDialogOpened` event and returns a typed chooser whose `setFiles` operation uses BiDi `input.setFiles`. Download subscribes to BiDi `browsingContext.downloadWillBegin` and `browsingContext.downloadEnd`, resolves at start with the URL and suggested filename, and exposes `saveAs` to wait for completion and copy the file. Chromium uses the injected BiDi mapper for file chooser and download handling, never a CDP implementation. Web-specific payloads and commands remain in `proto/surfaces/web/v1/web.proto`.
