...
- Rust crates by syncing workspace Cargo versions, verifying every Rust package and internal dependency against the tag, and running `scripts/publish-crates.sh publish full` with `ALLOW_EXPERIMENTAL_SURFACES=1` and `CARGO_REGISTRY_TOKEN`; the full profile is dependency-ordered and keeps every core, CLI, and plugin crate aligned with the release tag. The workflow currently sets `CARGO_PUBLISH_ALLOW_DIRTY=1` because version sync edits manifests during the job
...
- Chromium launch enables a DevTools endpoint only to bootstrap and transport the injected WebDriver BiDi mapper; that endpoint is not an automation API for Allwright operations.
- The implemented web plugin runtime path tracks browser sessions with engine session ids and mapper-backed BiDi context information.
- The initial page is discovered with BiDi `browsingContext.getTree`; new tabs are opened with `browsingContext.create`, discovered with `browsingContext.getTree`, and closed with `browsingContext.close`.
- Chromium tab navigation runs through the injected WebDriver BiDi mapper using `browsingContext.navigate` in both the normal plugin path and the standalone web-surface runtime.
- Chromium injects the pinned `chromium-bidi` mapper artifact into a hidden mapper target and uses it for all subsequent browser automation.
...
- Hard rule for future Chromium web-plugin work: all browser automation must execute through WebDriver BiDi. This includes browsing-context discovery/creation/closing, navigation, click, hover, focus, fill, key input, selector checks, text reads, highlighting, screenshots, and hooks. CDP is permitted only for Chromium BiDi mapper bootstrap and mapper message transport; it must never perform lifecycle, DOM, input, screenshot, or hook automation.
...
- Android app/locator APIs expose the Android-applicable shared context actions from web: `click`, `count`, `focus`, `fill`, `press`, `textContent`, `innerText`, `waitForSelector`, and `screenshot`; `hover` and `highlight` remain web-only unless the Android runtime gains real visual equivalents.
- `@allwright.dev/vitest/config` exports `allwrightVitestConfig(...)`, which patches Vitest's global setup list to own the managed Allwright server once per run; user global setup entries stay supported and are ordered inside Allwright's lifecycle.
...
- Snapshot `mode` accepts `default` (also the omitted/empty protocol default), `ai`, `autoexpect`, or `codegen`. AI includes rendered content as well as accessible content and generic elements; rendered pointer-receiving nodes have an optional `aria-ref` field matching an injected DOM attribute. References persist per document for unchanged element/role/name, are replaced on semantic changes, and stale owned attributes are removed on the next AI capture. Other modes never mutate references. `autoexpect` requires both accessibility and rendering; `codegen` uses default semantics with standard serialization, never Playwright pattern syntax.
- The shared `AccessibilitySnapshotCommand` and `AccessibilitySnapshotCapturedEvent` live in `proto/core/v1/context.proto`, routed through the shared `ContextSession` oneofs. Their package, names and field numbers remain unchanged. Core delegates to the target web or Android plugin.
- `rust/allwright-surface-web/src/accessibility.js` owns composed-DOM collection. `src/accessibility.rs` executes it using BiDi on Chromium and Firefox, enumerates all iframe browsing contexts, and serializes the resulting data as JSON or standard block YAML. The shared SDK `accessibility_yaml` serializer uses JSON scalar escaping in YAML block collections and quotes all strings so YAML 1.1/1.2 parsers preserve types. No DOM inspection through CDP, Playwright renderer syntax, or client-side snapshot implementation is permitted.
...
- `typescript/vitest/tests/accessibility.spec.ts` is an opt-in public fixture/transport/installed-runtime regression test. Enable with `ALLWRIGHT_TEST_BROWSER=chromium` or `firefox`; `ALLWRIGHT_TEST_LIVE_SITE=1` exercises the reported Form Inputs site. Direct plugin tests alone do not verify the client-to-engine path.
...
## Web Semantic Locators

- Rust, Go, Java, Python, TypeScript, and the lazy Vitest page fixture expose role, text, label, placeholder, alt-text, title, and test-ID locator builders. All builders can be chained with CSS/XPath locators. Locator filters support `has`, `hasNot`, `hasText`, `hasNotText`, and `visible`; role options support `name`, `exact`, `includeHidden`, `checked`, `disabled`, `expanded`, `selected`, `pressed`, and `level`. Locators also expose `first`, `last`, and `nth` (Python first/last are properties).
- Semantic locator transport uses `aw=` followed by a JSON-quoted JSON object, alongside the existing quoted CSS/XPath segments. This travels through the existing shared selector string fields without a proto/ABI change. Clients only construct/encode selectors; `rust/allwright-surface-web/src/selectors.js` owns all evaluation, executed through BiDi on both Chromium and Firefox. It reuses `accessibility_semantics.js` for accessible role/name/hidden semantics; `includeHidden` name computation is opt-in and does not change snapshot defaults.
- Nested `has`/`hasNot` locators must belong to the same page and evaluate relative to each candidate. Semantic/CSS queries traverse open shadow roots; XPath does not. Frame-locator APIs, configurable test-ID attributes, and complete Playwright accessibility conformance are not implemented. See `rust/allwright-surface-web/SELECTORS.md` for the supported contract and limits.
- Client builders live in `client_web_locators.rs`, `go/web_locators.go`, Python `_web_locators.py`, Java `WebLocators`/`WebSelectorSupport` and dedicated option classes, and TypeScript `web-locators.ts`. Vitest must defer locator construction and resolve nested lazy filter locators against the same realized page.
- `rust/allwright-surface-web/tests/selectors.rs` runs the browser resolver regression fixture with `--ignored`; set `ALLWRIGHT_TEST_BROWSER=firefox` for Firefox. `typescript/vitest/tests/selectors.spec.ts` tests the public fixture/server/plugin path when `ALLWRIGHT_TEST_BROWSER=chromium|firefox` is set.

## Negation

- Vitest Allwright page/app and locator expectations support `.not` and the callable `.not()` alias for all four existing matchers, including Android expectations. `typescript/vitest/src/expectations.ts` owns matcher types, immutable negation, and assertion polling; the main barrel supplies shared defaults and keeps ordinary Vitest assertions unchanged. Other language clients do not currently own an assertion framework.
- Negative assertions poll the inverted predicate. Web visibility queries count `filter({ visible: true })` matches, so absence/hidden nodes satisfy negative visibility without using a positive visibility wait. Android uses count plus the existing native visibility check; only the explicit `selector found but not visible:` error is treated as hidden. Other command errors cannot satisfy negation. Text negation requires a successful read. Nested command timeout hints are bounded by the assertion's remaining budget.
- All web clients and the lazy Vitest locators expose element-identity exclusion (`not(other)`, Go `Not`, Rust `not(&other)`, Python `not_`). Exclusion preserves scope/order and chaining, rejects cross-page locators, and is distinct from descendant `hasNot` filters. The web plugin owns the `exclude` semantic segment; mobile-native locator exclusion is not implemented.
- `typescript/vitest/tests/negation.spec.ts` checks web/Android negation, retries, failures, regex state, and command budgets without launching a browser. The opt-in `selectors.spec.ts` includes actual hidden/removal transitions and lazy locator exclusion through the engine.

## Android Accessibility Snapshots

- Android app contexts expose the same snapshot options/method names as web in Rust, Go, Java, Python, TypeScript and the Vitest `androidApp` fixture. Default JSON and standard YAML share a version-1 `documents` tree. The SDK owns only serialization; Android hierarchy collection, native role/name/state mapping and reference resolution live in `rust/allwright-surface-mobile-android/src/accessibility.rs`.
- Android snapshots derive from read-only ADB UI Automator XML. They do not inject attributes into the app or alter the returned source XML. Native metadata includes package/activity, class/resource ID, bounds and positional absolute XPath. This is an approximation of accessibility semantics, not a browser ARIA tree. WebViews are represented only as exposed in the native hierarchy. Password node names are redacted.
- `default` and `codegen` retain the hierarchy; `autoexpect` omits zero-area nodes while retaining their rendered descendants; `ai` adds synthetic `aria-ref` fields to enabled, rendered clickable/focusable/editable nodes. Use `app.locator("ref=<id>")` as a standalone locator for existing Android actions/reads. References cannot be chained with another selector.
- References are scoped to unique connection and app-context IDs. The plugin caches reference → positional `/hierarchy/node[N]/...` XPath. Each lookup obtains fresh XML and checks the complete source plus foreground package/activity. Any observed mismatch invalidates the generation, and old references fail as stale; foreign sessions fail before device work. Unchanged AI captures reuse references. Cache entries expire after ten minutes, are bounded to 128 app contexts, and are removed on app close. Native hierarchy identity is observational: identical recycled nodes and changes between dump and input cannot be detected atomically.
- Dynamic libraries remain resident after lazy first use so plugin-owned session caches survive ABI calls. A changed library file requires restarting the engine. Core still owns no Android reference semantics.
...
- Core owns the generic hook lifecycle plus context-scoped chunked file staging. Public upload/download paths are always client-local: clients read/write those paths, core transfers bytes through `UploadFileChunkCommand` / `ReadFileChunkCommand`, and surface commands carry opaque staged-file ids. Never resolve a public client path in the server or a surface plugin. Surface-specific registration/result payloads belong to their surface proto: web hook messages live in `proto/surfaces/web/v1/web.proto`, while mobile file-chooser/download messages live in `proto/surfaces/mobile/v1/mobile.proto`.
- Web hook types include new-page, file-chooser, and download. New-page snapshots top-level page ids and records the source page. File-chooser sessions set BiDi's file prompt behavior to `dismiss`, subscribe to `input.fileDialogOpened`, and return a typed chooser whose `setFiles` operation uses BiDi `input.setFiles`; the OS picker must never be shown or automated through AppleScript, UI Automation, or similar native tooling. Download registration must set BiDi `browser.setDownloadBehavior` to `allowed` with an explicit staging destination before the triggering action, so no native Save As dialog is possible; fresh Chromium/Firefox automation profiles must also disable download prompting and automatic download bubble/panel opening. Download subscribes to BiDi `browsingContext.downloadWillBegin` and `browsingContext.downloadEnd`, resolves at start with the URL and suggested filename, and exposes `saveAs` to wait for completion and copy the file. All Chromium automation—including context discovery/creation/closing, navigation, script execution, input, screenshots, and hooks—must use the injected WebDriver BiDi mapper. Direct CDP is restricted to bootstrapping that mapper and carrying BiDi messages to and from its hidden target; never expose or add a separate CDP automation command. Web-specific payloads and commands remain in `proto/surfaces/web/v1/web.proto`.
- Public APIs are scoped to the surface object where the event occurs: web hooks register on `Page`, while Android file hooks register on `AndroidApp`. TypeScript, Python, Java, and Rust expose the same generic `registerHook` / `register_hook` method on both owners. Go uses the single generic `RegisterHook(ctx, owner, hookType)` function because it does not support generic methods. Waiting returns the hook type's normal typed result.
- Android app contexts support the same two-phase file-chooser and download hook shape but reject new-page hooks. The Android plugin detects the system picker, stages client-uploaded bytes into public device storage through ADB, observes new public Downloads files, and pulls completed downloads into server staging before core streams them back to the test client. Standard DocumentsUI single-file selection and `/sdcard/Download` are the initial supported scope; private app storage and OEM-specific picker behavior must fail clearly rather than pretending to be portable.

## Dialog Hooks

- All five web clients and lazy Vitest pages expose a typed dialog hook through the existing generic registration/wait lifecycle: TypeScript/Python `hooks.dialog`, Go `Hooks.Dialog`, Java `Hooks.DIALOG`, Rust `DIALOG`. Results expose type/kind, message, default value, accept (optional prompt text), and dismiss. Empty text differs from omitted text; omitted text preserves the prompt default. Text on a non-prompt or dismissal is rejected. Waiting, accept, and dismiss expose the normal command timeout option across clients (Rust uses its `_with_options` methods). Registration intentionally has no timeout: cancellation could orphan a plugin subscription. Android rejects dialog registration.
- Web-owned `RegisterDialogHook`, `DialogHookResult`, `HandleDialogCommand`, and `DialogHandledEvent` live in `proto/surfaces/web/v1/web.proto`. Core only routes registration, polling, and handling through the SDK/plugin boundary. Dialog handling is intentionally not retried, to avoid accepting a subsequent prompt twice.
- The plugin retains each page's subscribed connection after handling and reuses it for the next one-shot registration, including already-buffered prompt-open events. This preserves synchronous sequences such as `alert('first'); alert('second')`. Page/browser cleanup drains bounded consecutive prompt chains before releasing the subscription, rather than dismissing only the first prompt. The subscription is released only on page/browser close. Handling is intentionally not retried, including when a command deadline is supplied, to avoid accepting a subsequent prompt twice.
- `rust/allwright-surface-web/src/dialogs.rs` owns page-scoped one-shot state, duplicate registration checks, owner validation, and subscription cleanup on handling/page/browser close. Chromium uses mapper BiDi; Firefox uses native BiDi. Both subscribe to `browsingContext.userPromptOpened` and handle through `browsingContext.handleUserPrompt`. Event queues match context IDs so hooks cannot steal another page's event.
- Core applies dialog handling options as single-attempt deadlines rather than retry policies. The synchronous dynamic-plugin ABI call for dialog handling must run through `spawn_blocking`, not `block_in_place`, so the Tokio deadline remains pollable. A deadline bounds the client's operation but must not replay side effects; timeout errors explicitly warn callers not to retry blindly.
- Dialog-aware focus retains the normal selector-chain resolver (including semantic `aw=` segments) on Chromium and Firefox; do not replace it with a single CSS/XPath query just to make script evaluation interruptible.
- Browser sessions leave JavaScript alert/confirm/prompt dialogs open (`ignore` prompt behavior). Expected dialogs must have a hook registered before the triggering action. `input.performActions` and explicitly opted-in void `script.evaluate` input actions (press, fill, focus, Firefox hover) yield on a matching prompt-open event so the sequential client stream can wait and handle it; the browser finishes suspended input after the modal is handled. Query/read evaluations do not opt in and must never receive synthetic data. Firefox action evaluation retains detached-frame validation. Navigation-triggered/before-unload dialogs are outside this input-action contract. No client parallel-command workaround or CDP automation is used.
- `typescript/core/tests/dialogs.test.ts` covers transport/default/empty/error cases. `typescript/vitest/tests/dialogs.spec.ts` exercises real client/engine/plugin accept/dismiss/prompt behavior, consecutive-dialog buffering and cleanup, real handling deadlines, chain-aware semantic focus, keydown/keyup/input/change/focus/mouseover handlers, subsequent input and ordinary action errors with `ALLWRIGHT_TEST_BROWSER=chromium|firefox`.
