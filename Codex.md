...
- The first hook type is the web new-page hook. The web plugin snapshots top-level page ids at registration and records the source page, then resolves a page absent from that baseline whose opener is the source page for both Chromium and Firefox. Core converts the returned opaque web page handle into a normal managed context session.
