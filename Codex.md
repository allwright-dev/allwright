...
- The Rust singleton client currently defaults to `http://127.0.0.1:50051`, also respects `ALLWRIGHT_SERVER_ADDR`, can be redirected in-process with `allwright::set_server_addr(...)`, and now auto-installs or reuses the matching-version local CLI/plugin/server when needed for a local address.
...
- The Go singleton client currently uses `ALLWRIGHT_SERVER_ADDR` or falls back to `127.0.0.1:50051`, and now auto-installs or reuses the matching-version local CLI/plugin/server when needed for a local address.
...
- The Java singleton client currently uses `ALLWRIGHT_SERVER_ADDR` or falls back to `127.0.0.1:50051`, supports in-process override with `Allwright.setServerAddr(...)`, and now auto-installs or reuses the matching-version local CLI/plugin/server when needed for a local address.
...
- The Python singleton client currently uses `ALLWRIGHT_SERVER_ADDR` or falls back to `127.0.0.1:50051`, supports in-process override with `set_server_addr(...)`, and now auto-installs or reuses the matching-version local CLI/plugin/server when needed for a local address.
...
- The TypeScript singleton client currently uses `ALLWRIGHT_SERVER_ADDR` or falls back to `127.0.0.1:50051`, also supports in-process override with `setServerAddr(...)`, and now auto-installs or reuses the matching-version local CLI/plugin/server when needed for a local address.
