use allwright_client::{LaunchOptions, launch_chrome, ping, set_server_addr, shutdown};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match args.command {
        PlaygroundCommand::Ping => {
            let response = ping().await?;
            println!("ping response: {response}");
        }
        PlaygroundCommand::BrowserSession => {
            println!(
            ...
            let initial_pong = initial_tab.ping("rust-playground-tab-1").await?;
            println!(
                "[{}] tab session pong: {}",
                initial_tab.session_id(),
                initial_pong
            ...
            for tab_index in 0..args.tabs {
                ...
                let tab_pong = tab
                    .ping(format!("rust-playground-tab-{tab_number}"))
                    .await?;
                println!("[{}] tab session pong: {}", tab.session_id(), tab_pong);
                additional_tabs.push(tab);
            }

            let browser_pong = browser.ping("rust-playground").await?;
            println!("[{}] session pong: {}", browser.session_id(), browser_pong);
